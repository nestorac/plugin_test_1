...
@export var special_powers := [] setget _set_special_powers # (Array, Resource)
@export var special_power_actions := [] setget _set_special_power_actions # (Array, String)
...
func _process(_delta):
	...
	if aim_at_ground:
		...
		ray_cast.target_position = get_node(camera_path).project_ray_normal(mouse_pos) * 1024.0
		ray_cast.set_collision_mask_value(5, true)
		ray_cast.force_raycast_update()
	
	# If no aimable surface, cast to the WorldBoundaryShape3D under the character's feet (physics layer 6)
	if !aim_at_ground || !ray_cast.is_colliding():
		ray_cast.set_collision_mask_value(5, false)
		ray_cast.set_collision_mask_value(6, true)
		ray_cast.force_raycast_update()
		ray_cast.set_collision_mask_value(6, false)
